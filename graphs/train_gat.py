...
def accuracy(logits, labels):
    ...
    #print(confusion_matrix(labels.detach().cpu().numpy(), indices.detach().cpu().numpy()))
...
def main(args):
    ...
    # create model
    heads = ([args.num_heads] * (args.num_layers-1)) + [args.num_out_heads]; print(heads)
    model = GAT(g,
                args.num_layers,
                num_feats,
                args.num_hidden,
    ...
    print(f'GAT Training time avg : {np.mean(train_dur):.6f} seconds | total: {sum(train_dur)} seconds')
